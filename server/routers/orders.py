...
from server.crud.orders import get_orders, add_order, check_order, search_order, update_order_state
...
def update_order_status(order_id: int, status: str, db_session: Session = Depends(get_db)):
    ...
    if order:
        updated_order = update_order_state(order, status, db_session)
        if updated_order:
            return {"message": "Order updated successfully."}
        else:
            raise HTTPException(status_code=500, detail="Invalid transition")
    else:
